...
class Parser(object):
    ...
    def cond_quantile(self, df, cond, n_quantiles):
        ...
        res_arr = numeric.quantilize_without_nan(df[cond], n_quantiles=n_quantiles, axis=1)
    ...
    def to_quantile(self, df, n_quantiles=5, axis=1, mask=None):
        ...
        res_arr = numeric.quantilize_without_nan(df, n_quantiles=n_quantiles, axis=axis)
        res = pd.DataFrame(index=df.index, columns=df.columns, data=res_arr)
        return res
